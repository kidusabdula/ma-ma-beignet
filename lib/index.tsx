...
//     /**
//      * Configuration options for the SWR (Stale-While-Revalidate) data fetching library.
//      * If provided, these options will be used to customize the behavior of SWR.
//      * @type {SWRConfiguration | undefined}
//      * @default {revalidateOnFocus: false, revalidateIfStale: false, shouldRetryOnError: false, suspense: true,}
//      */
//     swrConfig?: SWRConfiguration,
// }>

// export const FrappeProvider = ({ url = "", tokenParams, socketPort, siteName, enableSocket = true, children, swrConfig }: FrappeProviderProps) => {

//     const frappeConfig: FrappeConfig = useMemo(() => {
//         const frappe = new FrappeApp(url, tokenParams)
//         const _swrConfig: SWRConfiguration = {
...
//             enableSocket,
//             socketPort,
//             swrConfig: _swrConfig
//         }

//     }, [url, tokenParams, socketPort, enableSocket, JSON.stringify(swrConfig)])
...
//  * @param configs - [Optional] Configuration options for the hook
...
//  */
// export const useFrappeAuth = (options?: SWRConfiguration, configs?: FrappeAuthConfig): {
...
//     const { url, db, swrConfig } = useContext(FrappeContext) as FrappeConfig

//     const swrResult = useSWR<FrappeDoc<T>, Error>(swrKey === undefined ? getRequestURL(doctype, url, name) : swrKey, () => db.getDoc<T>(doctype, name), { ...swrConfig, ...options })
...
//     const swrResult = useSWR<T[], Error>(swrKey === undefined ? `${getRequestURL(doctype, url)}?${getDocListQueryString(args)}` : swrKey, () => db.getDocList<T, K>(doctype, args), { ...swrConfig, ...options })
...
//     }
//     const swrResult = useSWR<number, Error>(swrKey === undefined ? getUniqueURLKey() : swrKey, () => db.getCount(doctype, filters, cache, debug), { ...swrConfig, ...options })
...
//     const { call, swrConfig } = useContext(FrappeContext) as FrappeConfig
//     const urlParams = encodeQueryData(params ?? {})
//     const url = `${method}?${urlParams}`

//     const swrResult = useSWR<T, Error>(swrKey === undefined ? url : swrKey, () => call.get<T>(method, params), { ...swrConfig, ...options })
